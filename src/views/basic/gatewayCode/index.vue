<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="formatedData" size="small" border style="width: 100%;">
      ...
      <el-table-column prop="code" label="渠道码"/>
      <el-table-column prop="msg" label="渠道码说明"/>
      <el-table-column prop="isNormText" label="是否标准码"/>
...
export default {
  ...
  computed: {
    formatedData() {
      const data = []
      this.data.map(item => {
        this.isNormMap.map(isNormItem => {
          if (isNormItem.id == item.isNorm) {
            item.isNormText = isNormItem.label
          }
        })
  ...
  methods: {
    parseTime,
    beforeInit() {
      this.url = 'basic/gatewayCode/queryPage'
    ...
    subDelete(id) {
      ...
      doGet('basic/gatewayCode/delete?id=' + id).then(res => {
