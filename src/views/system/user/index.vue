      ...
      <el-table-column label="操作" width="300px" align="center">
        <template slot-scope="scope">
          ...
          <el-popover
            ...
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="$refs[scope.row.id].doClose()">取消</el-button>
              <el-button :loading="delLoading" type="primary" size="mini" @click="resetPwd(scope.row.id)">确定</el-button>
            </div>
            <el-button slot="reference" :disabled="scope.row.id === 1" type="danger" size="mini">重置密码</el-button>
          </el-popover>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    resetPwd(id) {
      this.delLoading = true
      doPost('api/users/resetPwd?id=' + id).then(res => {
        this.delLoading = false
        this.init()
        this.$notify({
          title: '重置密码操作成功,该用户的密码重置为123456',
          type: 'success',
          duration: 2000
        })
      }).catch(err => {
